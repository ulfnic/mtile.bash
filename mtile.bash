...
echo "display=vdisplay_${vdisplay_id}"
			try_display && break
		done
	fi
	...
	if [[ ! $display_set ]]; then
		for (( display_id=$display_count; display_id > 0; display_id-- )) do
			local -n "display=display_${display_id}"
echo "display=display_${display_id}"
			try_display && break
		done
	fi
